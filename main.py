...
@app.get("/")
def read_root():
    return {"Welcome to the world of Shakespeare!"}

@app.get("/generate/{length}")
async def generate_text(length: int):
    if (length < 0 or length > 1000):
        return {"Error": "Length should be a positive integer less than or equal to 1000"}
    
    inp = torch.zeros((1,1), dtype=torch.int)     # 1 batch, 1 elements - starting with newline
    res = model.generate(inp,length)
    return {"text": decode(res[0].tolist())}
